...
public final class Statement1Parse1 extends Statement1 {
    ...
    private static void parseIf(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isCondition(condition),
                "Expected valid condition, but was " + condition + ".");
        Statement.Condition c = parseCondition(condition);
        String then = tokens.dequeue();
        Reporter.assertElseFatalError(then.equals("THEN"),
                "Expected THEN, but was" + then + ".");
        ...
        if (tokens.front().equals("ELSE")) {
            ...
            Reporter.assertElseFatalError(tokens.front().equals("END"),
                    "Expected END IF, but was " + tokens.dequeue() + ".");
            Reporter.assertElseFatalError(tokens.front().equals("IF"),
                    "Expected END IF, but was " + tokens.dequeue() + ".");
            s.assembleIfElse(c, block, elseBlock);
        } else {
            Reporter.assertElseFatalError(tokens.front().equals("END"),
                    "Expected END IF, but was " + tokens.dequeue() + ".");
            Reporter.assertElseFatalError(tokens.front().equals("IF"),
    ...
    private static void parseWhile(Queue<String> tokens, Statement s) {
        ...
        Reporter.assertElseFatalError(doString.equals("DO"),
                "Expected DO, but was" + doString + ".");
        Statement block = s.newInstance();
        block.parseBlock(tokens);
        Reporter.assertElseFatalError(tokens.front().equals("END"),
                "Expected END WHILE, but was " + tokens.dequeue() + ".");
    ...
    public void parse(Queue<String> tokens) {
        ...
        if (tokens.front().equals("IF")) {
            ...
            Reporter.assertElseFatalError(false,
                    "Expected statement, but was " + tokens.front() + ".");
            //Should never happen if there is a valid statement
        }
