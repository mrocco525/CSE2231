...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (last >= left) {
            int right = left + 1;
            int smallest = left;
            if (last > left && order.compare(array[right], array[left]) < 0) {
                smallest = right;
            }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        int leftPosition = 2 * top + 1;
        int rightPosition = 2 * top + 2;
        if (array.length > 1 && leftPosition < array.length) {
            heapify(array, leftPosition, order);
            heapify(array, rightPosition, order);
