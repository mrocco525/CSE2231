 ...
 * </pre>
 *
 * @authors Caden Sweeney, Mason Rocco
 *
 */
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        int bucket = mod(keyHashCode, this.hashTable.length);
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int keyHashCode = key.hashCode();
        int bucket = mod(keyHashCode, this.hashTable.length);
        Map.Pair<K, V> returnValue = this.hashTable[bucket].remove(key);
        this.size--;
        return returnValue;
    }
    ...
    public final Pair<K, V> removeAny() {
        ...
        int searchInBucket = 0;
        while (this.hashTable[searchInBucket].size() == 0) {
            searchInBucket++;
        }
        Map.Pair<K, V> toReturn = this.hashTable[searchInBucket].removeAny();
        this.size--;
        return toReturn;
    ...
    public final V value(K key) {
        ...
        V returnValue = this.hashTable[bucket].value(key);
        return returnValue;
    }
    ...
    public final boolean hasKey(K key) {
        assert key != null : "Violation of: key is not null";
        int bucketToSearch = mod(key.hashCode(), this.hashTable.length);
        boolean mapHasKey = this.hashTable[bucketToSearch].hasKey(key);
        return mapHasKey;
    }
